 ...
 * @api public
 */
var Person = function(name,id,recommendations) {
    this.id    = id || uuid.v1()
  , this.name  = name || null
  , this.items = []
  , this.recommendations = recommendations
  , this.mat   = recommendations.matrices
  , this.db    = recommendations.client
  ;
  return this;
}
Person.prototype.addRedis = function(callback) {
  // this.db.zadd()
}
Person.prototype.addItem = function(name,score,id) {
  this.items.push(new Item(name,score,this,id));
}
 ...
 * @api private
 */
var Item = function(name,score,person,id) {
    this.id   = id || uuid.v1()
  , this.name = name || null
  , this.score= parseFloat(score) || null
  ;
  person.mat.pushItem(this.name, person.name, this.score);
}
